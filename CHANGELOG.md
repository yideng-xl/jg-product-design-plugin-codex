...
## 2026-09-09 · 统一交付地图维护流程

- 集管、网管保留独立入口，共用字段核对、地图更新、验证与发布规则。
- 产品路径、视图和本地配置差异集中维护，检查脚本供两条产品线复用。
- README 统一为用途、入口和调用示例；业务文档地址留在产品工作区。
...
## 2026-09-09 · 网管固定数据源与页面简化

- 网管页面数据源固定为用户指定的金山文档链接，每次使用用户提供的最新导出表。
- 移除“本期项目明细与覆盖依据”区块，项目核对信息保留在本地文件。
- 同步更新网管 skill、字段规则与 README，避免重建时恢复旧文件名或明细区块。
