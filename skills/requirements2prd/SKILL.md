...
description: Use when the user has approved using this skill to turn rough product needs into PRD material, or to discuss B-side module design, existing-module optimization, refactoring, productization, standardization, historical debt, "缝合怪", "项目拼凑", "聊需求", "做 PRD", "产品方案", or "新模块设计".
---
...
本 skill 的所有"关键判断点"(0 号问题、5W 拆解、约束清单、数据模型、状态机、视图维度、MVP 范围、挂起话题),**逐项用 `向用户提问并等待明确确认` 工具明确确认**,不要默默推进。

- 不许"我以为你就是想要 X,所以接着做了"
- 不许"这个点我自己判断了一下,后面再说"
- **每个判断点都要明确"我们达成了什么共识"**,落到文档里

判断"是否需要确认":如果你内心 P(对) > 80% 还有疑问,或者用户没给过明确表态 → 必须 向用户提问并等待明确确认。
...
> 安全规则要在需求阶段进入 PRD,不能等原型自查时再补。`proto-check` 是闸门,不是安全需求的来源。

细化需求时读取姐妹 skill 的 [`../proto-check/assets/日常迭代安全自查表.md`](../proto-check/assets/日常迭代安全自查表.md),按其中 14 个 `S-*` 触发场景扫描本次新增 / 修改范围。

执行时机:先把本轮 What、How、边界和约束确认清楚,再做安全触发扫描;必须在输出 PRD、进入原型或开发之前完成。

### 执行边界

- **按触发场景检查**:只展开本轮命中的 S-* 项。登录、账号口令、会话、系统级角色、全局审计字段、TLS/SSL、端口和中间件加固等框架能力,本轮未改动时不重复写;确有改动时标记“框架项变更”,转专项评审。
- **普通字段不重复检查**:普通名称、备注、查询条件仍走表单字段规则。只有可配置接口、连接及请求参数控件才命中 S-03。
- **产品规则先确认**:逐项确认用户能配置或执行什么、允许和禁止什么、影响范围、失败处置、权限、确认、结果和留痕。业务事实不清楚时问用户,不得代填“合理默认值”。
- **check 规则必须展开**:存在页面输入或操作时,PRD 必须逐条列出检查对象、允许/禁止条件、前端执行点、后端执行点和失败处置。只有“安全校验”“过滤特殊字符”“前后端 check”等概括描述不能进入原型。
- **实际入口写准确**:没有前端页面的开放接口、回调、消息接入和供应链检查,写网关、接入层、服务端、构建或供应链流程,不生造前端 check。
- **产品与实现分开**:PRD 写清产品规则和验收口径;服务端强制、注入防护、加密、防重放、组件漏洞等列入“技术评审/测试承接”,不能因为规则已写就标成安全完成。

### PRD 固定承载方式

本轮命中安全场景时,在 PRD 增加“附录 D:本轮安全需求”。功能章节保留与业务行为直接相关的简要规则,完整规则放附录,两处用 S-* 编号互相引用。
...
### D.2 产品规则与 check 规则
| 编号 | 产品规则 | check 对象与允许/禁止条件 | 前端执行 | 后端/实际入口执行 | 失败提示或处置 | 技术评审/测试承接 |
|---|---|---|---|---|---|---|
...
没有命中项时不生成空表,在 PRD 范围说明中记录“本轮已扫描日常安全触发场景,未命中”。命中项的 check 规则尚未确认完整时,PRD 保持“待确认”,不得流转到原型阶段假定补齐。

---

## 设计规范涉及的产品决策(命中场景时必查)

设计规范里有一部分规则需要产品经理先作决定。细化需求时读取姐妹 skill 的 [`../proto-check/assets/产品设计自查表.md`](../proto-check/assets/产品设计自查表.md) 和 [`../proto-check/assets/七大易用原则量化标准.md`](../proto-check/assets/七大易用原则量化标准.md),只展开本轮命中的场景。规则原文以这两份文件为准,本 skill 只负责确认产品参数并写进 PRD。

| 命中场景 | 读取规则 | PRD 需要产出的参数 |
|---|---|---|
| 表格操作列 | R-08、P-10 | 权限分支、业务条件分支、申请入口、需提示时的文案 |
| 数据筛选日期 | Z-时间-01 | 可选范围、数据可查询起点、最大筛选跨度、起止联动 |
| 定时任务 | Z-时间-02 | 开始与结束、永久生效、到期状态 |
| 表格排序 | Z-列表-04、T-03、C-06 | 是否允许手动排序、可排序列、默认排序列和方向 |
| 数据导出 | Z-列表-08、E-07 | 导出字段、格式和范围 |
| 网管下级页面 | Z-下级页-01 | 产品线、返回目标、父页面上下文保留、未保存内容处理 |

这些内容属于产品行为。Tooltip 移入/移出毫秒数、配色、间距等 UI 实现参数不写进 PRD。
...
**怎么用**:遇到岔路,**先列至少 3 个方案 + 一张权衡表(各自的代价/收益),再用 `向用户提问并等待明确确认` 让用户选**。强制要求:其中至少一个是"做减法/反着来"的方案。
...
### I. 本轮安全需求
- [ ] 已按 14 个 S-* 场景扫描本次新增 / 修改范围,只展开命中项
- [ ] 命中项已逐条写清产品规则、check 规则、前端与后端/实际入口执行点、失败处置
- [ ] 只有“安全校验”等概括描述的条目仍标待确认,未流转到原型
- [ ] 技术评审 / 测试承接与产品规则分开,未把运行防护写成产品已完成
- [ ] 未改动的登录、会话、TLS、端口等框架能力未重复展开;框架变更已转专项评审

### J. 设计规范相关产品决策
- [ ] 命中场景已读取对应 Z/F/T/D/C/R/E/P 编号,产品参数已写进功能项和验收标准
- [ ] 网管下级页面已读取 Z-下级页-01;集管未套用该项
- [ ] 纯 UI 实现参数没有写进 PRD
...
### 错 9:确认点跳过
"我以为对面就是这个意思,后面再说" —— 错。**逐项 向用户提问并等待明确确认 明确确认**。
...
- **附录 C**:术语表(中英对照 + 业务定义)
- **附录 D**:本轮安全需求(仅在命中 S-* 场景时生成,含触发识别、产品规则与 check 规则、技术/测试承接)
...
操作入口的权限与可用条件、日期边界、排序能力、导出范围和返回后的上下文保留属于产品规则,仍需写进对应功能项。按钮位置、Tooltip 延时、配色和间距才归原型 / UED。
...
7. **本轮安全需求**(PRD 附录 D;无命中时在范围说明记录已扫描未命中)

---

## 与下一步的衔接

PRD 完成且命中项的安全规则已确认后,进入姐妹 skill **`prd2prototype`**(本 plugin 内的另一个 skill),把 PRD 和本轮安全需求落成可评审的高保真原型。
