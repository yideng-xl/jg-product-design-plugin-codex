...
**分页完整结构(T-06 / E-08)**:数据量上限未知或超过 15 条的列表,生成时一次放齐 5 项:① 每页条数选择,固定选项 15/25/50/100/200;② 当前数据范围;③ 总条数;④ 页码与前后翻页;⑤ 指定页跳转。默认 25 条/页,展示区域不足时用 15 条/页。不能只画页码和总数后留到自查补齐。业务明确数据量不超过 15 条时可不分页。
...
> 完整标准见姐妹 skill 的 `../proto-check/assets/七大易用原则量化标准.md`(原型检查版 51 条,F/T/D/C/R/E/P;纯运行时 / 设计阶段 / 渲染主观 / 生产环境条目已删)与 `../proto-check/assets/产品设计自查表.md`。**做原型时就按它做,proto-check 事后会按同一份标准验收**——生成时遵守一条,验收时就少一条整改。下面只列生成时最容易违规的:

- [ ] 每个表单 / 弹窗 / 抽屉内**只有一个主按钮**(`.btn-primary`),落在最高优先级动作上;"取消"不用主按钮(F-01)
- [ ] 表格状态列一律用 `.tag`/`.status`,**不裸文本**表示状态(F-04)
- [ ] 产品原型完整展示已确认的操作入口,避免需求遗漏;是否收进【更多】由设计阶段决定,不作为产品原型自查项
...
- [ ] 查询区每个条件都能在列表找到对应字段;列表字段不要求全部提供查询条件(E-03);分页已同时包含每页条数 15/25/50/100/200、当前范围、总条数、页码与前后翻页、指定页跳转,默认 25 条/页,展示区域不足时用 15 条/页;业务明确不超过 15 条时可不分页(T-06/E-08)
- [ ] 通用空状态直接沿用公共组件,无需在页面中单独展示或写入 proto-note;只有特殊文案、引导按钮、业务限制或后续操作时才展示并说明(C-02)
- [ ] 异常分类和处置规则已确认时,原型展示原因与建议;尚不能归类时只记录待确认事项,不编造分类、建议或整改要求(C-03)
...
> 完整规则见 [`../proto-check/assets/日常迭代安全自查表.md`](../proto-check/assets/日常迭代安全自查表.md)。PRD 定规则,本 skill 落页面与说明,`proto-check` 按同一编号验收。

- [ ] 已读取 PRD“附录 D:本轮安全需求”,把每个命中 S-* 项映射到具体页面、控件和 proto-note
- [ ] 命中项已逐条列出 check 对象、允许/禁止条件、前端执行点、后端/实际入口执行点和失败处置;没有只写“安全校验”
- [ ] 用户可见的限制、提示、确认、状态和失败结果已落到原型;后端运行防护仍明确标“需技术评审/测试验证”
- [ ] S-01 不使用无边界的自由命令输入框;命令、选项、参数和禁止内容的规则清单已确认并写入 proto-note
- [ ] S-03 只检查可配置接口、连接及请求参数控件,普通业务字段不与既有字段规格表重复
- [ ] S-06 下载/导出文件名为 `页面名称_YYYYMMDD_HHmmss.扩展名`,如 `告警查询_20260825_143025.xlsx`
- [ ] 框架能力未改动时未重复画;框架项变更已单列专项评审承接
...
- [ ] 自定义列默认不提供;仅业务有特殊要求或表格列数大于 15 列时增加入口,并说明默认展示列、可选列、固定列;序号和操作列不可自定义(Z-列表-06)
