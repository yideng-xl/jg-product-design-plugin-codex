...
> 安全规则来自 PRD,原型负责把已确认的产品规则表达清楚。姐妹 skill `proto-check` 只做闸门检查,不能替代前置设计。

开始画页面前,读取 PRD 的“附录 D:本轮安全需求”和共享规则源 [`../proto-check/assets/日常迭代安全自查表.md`](../proto-check/assets/日常迭代安全自查表.md):

1. 按 S-* 编号把每个命中项映射到具体页面、控件、交互和 proto-note。
2. PRD 需要有命中项附录,或明确记录“已扫描、未命中”。没有安全触发识别,或命中项只写“安全校验”而没有规则清单时,先退回 `requirements2prd` 补齐,不得在画原型时自行编造。
3. 只承接本轮命中项。未改动的登录、账号、会话、TLS、端口等框架能力不重复画;框架项变更单列专项评审入口或说明。
4. 用户可见的限制、提示、确认、审批、执行状态、失败结果按 PRD 落到界面和交互;完整 check 规则、前后端/实际入口执行位置写进 proto-note。
5. 后端强制、权限绕过、注入防护、加密、防重放、组件漏洞等统一写“需技术评审/测试验证”。不得用前端 JS 模拟后宣称安全能力已实现。

多页面项目在 `conventions.md` 增加本轮安全映射表,让各页面和子任务使用同一口径;单页项目可放在本轮范围说明中。无论载体是什么,映射表都必须存在:

```markdown
| 编号 | 命中功能 | 页面/控件 | 用户可见规则 | check 规则及执行位置 | proto-note 位置 | 技术/测试承接 |
|---|---|---|---|---|---|---|
```
...
| `assets/editor/` | **原型编辑器源码**(跨平台,需装 Node):从本仓库 Release 下载 `prototype-editor.zip` → Mac 双击 `原型编辑器.app` / Win 双击 `原型编辑器.vbs` → 浏览器打开控制页 `http://localhost:47821/` → 选择原型 → 页面上修改需求便签或原型说明,自动写回该原型的 `data/annotations.js`。关掉页面后服务自动停止。发布包由 `assets/editor/pack.sh` 生成 |
...
**规范层级与冲突处理**:全局样式规范定义 token 和基础样式,组件规范定义控件结构与交互,场景化规范只组合已有组件。三者冲突时按“全局样式规范 > 组件规范 > 场景化规范”执行。场景需要的新能力先补进共享组件,再由页面引用;不得在单页改写组件视觉或交互形成局部版本。
...
**自定义列规则**:默认不提供自定义列入口。业务明确提出自定义列要求,或表格列数大于 15 列时,才增加入口;并在原型说明写清默认展示列、可选列和固定列。序号、操作列固定展示,不可自定义。列数不超过 15 列且没有特殊要求时,不放入口,也不用补“不支持自定义列”的说明。

**分页完整结构**:命中分页场景时读取共享规则 T-06 / E-08 和 Z-列表-12,按其当前参数一次画完整,不在本 skill 另存分页数量和默认值。
...
**需要说明的交互规则(铁律)**:页面存在非常用或不直观的交互逻辑时,在底部"原型设计说明"里放一张「按钮 / 交互事件清单」表(含弹窗 / 抽屉内部的):

- **新增页面**列出全部需要额外说明的交互;**修改既有页面**只列本迭代新增 / 修改的交互及其直接影响。
- **需要列**:字段联动、条件显隐、自动带值或清空、控件启用 / 禁用、状态变化、跨页面带参、特殊校验、操作后的非直观业务结果。
- **不用列**:新增、查看、编辑、删除、普通查询 / 重置、批量删除、弹窗打开 / 关闭、Tab / 图表切换、复选框全选、普通分页等清晰可见的常规操作。常规操作存在额外规则时只写额外规则,如"全选只选择当前页"或"关闭弹窗会清空已填写内容"。
- 普通录入 / 筛选控件不列。字段规格表已覆盖控件本身,清单不重复。
- 固定列:**位置 / 按钮·交互 / 触发后行为 / 校验·状态变化**。
- **位置列写全层级路径**:哪个 Tab → 哪个区块(section) → 哪个弹窗 / 抽屉(如「VLAN 明细 Tab · 筛选条」「终端地址 Tab · 新增弹窗」)。
- 原型辅助元素(需求标签 i 图标、原型说明区等非产品功能)不列。页面没有需要额外说明的交互时,整张清单连卡片一起不放。
...
**命中安全场景的说明方式**:每个命中的 S-* 项必须在对应页面 proto-note 中写清“产品规则 + check 规则及执行位置 + 失败处置 + 技术/测试承接”。存在页面输入或操作时,明确前端和后端都执行 check;没有前端页面时写清网关、接入层、服务端、构建或供应链流程。只写“已做安全校验”视为缺失。复杂 check 规则同样使用「逻辑表 + 测试用例 + 静态示例」,但静态示例只能说明产品表现,不能证明运行防护已经实现。
...
**实操**:每次改动前,Codex 应该主动问产品"上次有没有 push 并通知研发?",或者直接采用更保守的"先按合并处理,等通知前再统一升号"。**避免机械每次小改都升号**——版本爆炸会让评审同事追不上、变更记录被淹没。
...
4. **共性 / 全局说明的固定区(铁律)**:凡是"全产品统一、不随单个页面变"的说明,集中放在 index 一处,不在各业务页 proto-note 重复。系统权限 / 数据权限按共享规则 Z-整体-10 执行,具体链接和达标口径只在权威源维护。
...
**实操**:每次改动结束,Codex 回头问"这件事用一句业务话能讲清楚吗?":
- 能 → 一条摘要
...
**JS / CSS / 数据文件加查询版本破除浏览器缓存(铁律)**:原型部署到内网静态服务器后,评审同事打开页面看到的常是**浏览器缓存的旧文件**。在 HTML 的 `<script>` / `<link>` 标签 `src` / `href` 末尾加查询参数 `?v=x.y.z`,查询值变化后浏览器会重新拉取资源。

这里有两套版本,用途分开:

- 页面可见的 `v0.1 / v0.2 / v0.3.0...` 是**原型评审版本**,按本节的产品变更规则维护。
- CSS、JS、数据文件后的 `?v=` 是**工程修订号**,只用于破除缓存。它可以与原型评审版本采用不同粒度,不得写进业务版本变更记录。

每次交付前必须扫描 index 和全部业务页。一个原型交付内,所有本地 CSS、JS、数据文件引用统一使用同一个 `?v=` 值;发现多个值时,统一为本轮已使用的最新工程修订号。这个检查属于生成质量检查,交付前直接修正,不得留给产品自查评分。
...
**踩坑案例**:某次同一原型的首页和业务页分别使用 `0.1.16`、`0.1.19`,说明生成阶段漏了全量扫描。该问题应在交付前统一查询值,不应进入产品设计自查的不符合项。

### 本地 Git 版本管理(修改原型前必做)

每个产品迭代目录单独建立本地 Git,仓库范围覆盖本迭代的 PRD、原始资料和原型。默认不配置远程仓库、不 push;本地 Git 只用于记录修改、查看差异和恢复历史。

1. 修改前检查迭代目录是否已有 `.git/`;没有则先创建 `.gitignore`,执行 `git init`,提交一次“建立原型本地基线”。
2. 删除页面、说明、字段或交互前,先确认已有可回退提交;没有基线不得删除。
3. 用户每确认一项修改,完成实现和验证后提交一次;一个提交只对应一项业务修改。
4. 提交前检查 `git diff`,避免把无关文件混入;不得使用 `git reset --hard`、`git checkout --` 覆盖用户改动。
5. `.gitignore` 排除 `.DS_Store`、日志、Office 临时文件和依赖目录;历史 `.bak`、原始资料等业务文件保留。
...
**原型编辑器(独立工具,跨平台,壳是一个控制页)**:本仓库是编辑器的唯一源码和发布源。源码位于 `assets/editor/`,通过 `assets/editor/pack.sh` 生成 `dist/prototype-editor.zip`,再随本仓库 GitHub Release 发布。用户从 <https://yideng-xl.github.io/jg-product-design-plugin-codex/#editor> 下载并解压。压缩包包含自包含的 `原型编辑器.app`(serve.js/launcher.js/panel.html 在 `Contents/Resources/` 内)、`原型编辑器.vbs`(Windows)和 `原型编辑器.command`(Mac 兜底)。Mac 双击 App,Windows 双击 `.vbs`。需安装 Node.js。
...
- 控制页和编辑条上都有「使用说明」链接,统一指向本仓库 GitHub Pages(`https://yideng-xl.github.io/jg-product-design-plugin-codex/#editor`)。
...
1. 原型根目录必须有 `data/annotations.js`;已有文件永不覆写,没有时才创建空覆盖层。
2. 每个引用 `common.js` 的 HTML 页面都必须先引用 `data/annotations.js`;两者之间不能倒序。
3. 每个需求便签的 `.pt-pop-item`、每个需要产品手改的原型说明段或说明块都必须分配稳定且全项目唯一的 `data-anno-id`。
4. 富文本说明块加 `data-anno-rich`;纯文字不加。`data-anno-rich` 不能脱离 `data-anno-id` 单独出现。
5. 完成后运行 `node scripts/check-prototype-editable.mjs <原型目录>`。检查失败不得交付,也不得声称“原型可编辑”。
...
3. 编辑态判据只认 `location.hostname`:仅 `localhost` / `127.0.0.1` 可编辑;`file://` 和其它 http 主机均只读。不加任何"手动关掉开关"的常量或页面元素。
...
- **新增或调整的安全触发项、check 规则、前后端/实际入口执行位置、失败处置** → 更新 PRD 附录 D 和对应功能规则;运行防护同步进入技术/测试承接
...
- 本轮安全需求中的服务端强制、权限校验、注入防护、加密、防重放、限流、组件漏洞等技术/测试承接
- 列出本轮需要技术侧交付的能力清单
...
| **架构 / 后端** | 数据模型与接口是否合理 / 跨模块依赖 / 性能瓶颈 / 安全 check 规则能否在服务端独立执行 / 运行防护承接 |
| **运维 / SRE** | 部署变更 / 监控埋点 / 应急预案 / 容量评估 |
| **QA** | 验收标准是否可测 / 测试用例覆盖度 / 测试数据准备 / 安全规则绕过、注入、越权和边界测试 |
...
- 安全产品规则和 check 规则来自 PRD 附录 D 与 PRD 细则;技术/测试承接不得改写成已完成能力。
...
> 完整标准见姐妹 skill 的 [`../proto-check/assets/七大易用原则量化标准.md`](../proto-check/assets/七大易用原则量化标准.md)(原型检查版 51 条,F/T/D/C/R/E/P)与 [`../proto-check/assets/产品设计自查表.md`](../proto-check/assets/产品设计自查表.md)(89 条,Z)。两份文件是历史规则和当前规则的共同权威源;本节只保留执行步骤,不复述规则内容。

- [ ] 已完整读取两份权威源,按本轮页面和组件建立“适用规则编号清单”;未命中的条目标记不适用,不机械套用
- [ ] 每个适用编号均已落到界面、proto-note、字段规格表或待确认项;以权威源的“原型判定 / 原型检查口径”为准
- [ ] PRD 缺少产品参数时已记录待确认,原型没有自行补值、补状态或补权限分支
- [ ] 标为【说明】的要求已写进 proto-note 或字段规格表;运行时、生产环境和技术验证项没有用静态交互假装完成
- [ ] 交付前按同一编号运行 `proto-check`;整改只改权威源指出的规则或本轮页面,不在本节新增一套口径

### A3. 日常迭代安全规则(生成时即落实)

> 完整规则只维护在 [`../proto-check/assets/日常迭代安全自查表.md`](../proto-check/assets/日常迭代安全自查表.md)。PRD 定规则,本 skill 落页面与说明,`proto-check` 按同一编号验收。

- [ ] 已读取 PRD“附录 D:本轮安全需求”和共享规则源,把每个命中 S-* 项映射到具体页面、控件和 proto-note
- [ ] 用户可见规则按对应 S-* 编号落实;运行防护仍明确标为技术评审 / 测试承接
- [ ] 未命中的场景和本轮未改动的框架能力没有重复展开;规则缺口回到 PRD 确认,不在原型阶段另造口径

### B. 交互完整性
- [ ] 每个 href / onclick / 按钮的 destination 明确(能说出"点了去哪/干什么")
- [ ] 没有 href="#" 的兜底链接(除非是占位的 demo,且原型说明里标注了)
- [ ] 二次确认弹窗在所有"状态变更 / 批量"操作上都加了
- [ ] 删除确认已按共享规则 P-02 落实,本 skill 未另存一套文案
- [ ] 复杂联动逻辑没硬做交互,而是用「逻辑表 + 测试用例 + 静态示例」在原型说明里讲清(用例枚举到组合 / 边界,不用伪代码)
- [ ] 页面有不直观交互时,页底配「按钮 / 交互事件清单」表;新增页列全部需说明项,修改页只列本次新增 / 修改项及直接影响
- [ ] 清单只列字段联动、条件显隐、自动带值 / 清空、启用 / 禁用、状态变化、跨页面带参、特殊校验和非直观业务结果;增删改查、普通查询 / 重置、批量删除、弹窗开关、Tab / 图表切换、全选、普通分页等常规操作不列
...
- [ ] 数据权限已按共享规则 Z-整体-10 落实,本 skill 未另存链接或达标口径

### D. 列表设计
- [ ] 列宽做过预算,表头不会换行 / 数据列不会被挤
- [ ] 自定义列已按共享规则 Z-列表-06 判断是否适用并落实
...
### 多对象批量命令操作:先定安全边界,再画输入与结果

如果设计涉及"对多个对象同时执行命令"(如批量 SSH 远程操作多机),先按 S-01 确认命令输入方式和 check 规则。**禁止默认生成无边界的自由命令输入框。**

- **默认形态**:优先使用“已确认的命令模板/命令白名单 + 参数控件 + 设备列表 + 每台发送状态”。模板、命令、选项和参数范围来自 PRD,原型不自行补命令
- **确需人工输入时**:PRD 必须逐条列出允许的命令、选项、参数类型/长度/范围,以及禁止的连接符、重定向符、命令替换和多行输入;原型展示前端校验与失败提示,proto-note 明确后端按同一规则清单独立校验
- **执行前**:展示目标对象、数量和影响范围;按 PRD 落实权限、再鉴别/审批/确认、超时、并发和审计规则
...
**单设备交互式终端也必须执行 S-01**:可以保留终端形态和实时回显,但命令范围、前后端 check、失败提示、权限、确认和审计规则不能省略。原型只表达规则与用户反馈,后端防 Shell 注入和绕过仍由技术评审/测试验证。
...
11. **本轮安全需求页面映射与技术/测试承接清单**(按 S-* 编号与 PRD 附录 D 对齐)
...
- **评审前闸门**:姐妹 skill **`proto-check`** —— 按产品、UI 和日常安全三套规则检查;安全项仍按同一 S-* 编号核对
- **关键收尾**:第 8 步 —— 反写 PRD 细则、回写 PRD、生成研发对接版 Word、锁定本轮迭代、拉通设计 / 技术 / QA 确认
- **下一步**:安全闸门通过、原型评审完成并回写后进入开发 / 联调 —— 拿着 PRD 细则、PRD 终稿、研发对接版 Word、锁定的迭代范围、安全承接清单和各方确认凭据开工。**common.css 直接拿去用,组件结构 1:1 实现**
