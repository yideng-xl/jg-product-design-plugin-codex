...
**需要说明的交互规则(铁律)**:页面存在不直观的交互时,在底部"原型设计说明"里放一张「按钮 / 交互事件清单」表(含弹窗 / 抽屉内部的):

- **新增页面**列出全部需要额外说明的交互;**修改既有页面**只列本迭代新增 / 修改的交互及其直接影响。
- **需要列**:字段联动、条件显隐、自动带值或清空、控件启用 / 禁用、状态变化、跨页面带参、特殊校验、操作后的非直观业务结果。
- **不用列**:Tab 切换、复选框全选、普通分页、弹窗打开 / 关闭、点击结果与按钮名称一致的常用操作。常用操作存在特殊规则时仍要列,如"全选只选择当前页"或"关闭弹窗会清空已填写内容"。
...
- [ ] 清单只列字段联动、条件显隐、自动带值 / 清空、启用 / 禁用、状态变化、跨页面带参、特殊校验和非直观业务结果;Tab 切换、全选、普通分页、弹窗开关、含义明确的按钮点击不列
