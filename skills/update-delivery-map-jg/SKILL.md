...
description: 根据用户提供的最新集管月度进度表更新集管交付地图，核对项目映射和覆盖状态，重建地图并按已确认的配置发布预览。用于集管地图更新与重新发布；网管使用对应的独立入口。
---

# 更新集管交付地图

本入口属于“三、产品交付阶段”，本次产品设为 **集管**。

## 执行

1. 读取 [产品差异配置](../../references/delivery-map/products.md) 中的集管项，定位本次进度表、产品目录和地图。
