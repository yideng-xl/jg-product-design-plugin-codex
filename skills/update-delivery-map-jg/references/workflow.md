...
页面“数据源”固定指向 产品目录中配置的在线业务文档链接，不显示本地文件名。每次由用户从该文档提供最新表格，按本次文件更新；不自动抓取在线数据。

用户已取消环境表集成。流程不依赖 `delivery_details.json`、`build_delivery_details.py` 或 `render_delivery_panel.py`。顶部详情链接固定为 产品目录中配置的在线业务文档链接，除非用户提供新地址。
